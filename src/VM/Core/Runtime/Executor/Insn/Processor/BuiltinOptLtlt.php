...
class BuiltinOptLtlt implements OperationProcessorInterface
{
    ...
    private function compute(RubyClassInterface $leftOperand, RubyClassInterface $rightOperand): ?RubyClassInterface
    {
        ...
        if ($leftOperand instanceof Integer_ && $rightOperand instanceof Integer_) {
            $value = $this->computeNumberLeftShiftNumber($leftOperand, $rightOperand);
        }

        return $value;
    }

    private function computeNumberLeftShiftNumber(Integer_ $leftOperand, Integer_ $rightOperand): RubyClassInterface
    {
